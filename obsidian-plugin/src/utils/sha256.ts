 ...
 * Produces deterministic 64-character hex strings for strings in UTF-8.
 */
export function sha256(input: string): string {
  ...
  function str2binb(str: string): { bin: number[]; len: number } {
    const bin: number[] = [];
    const mask = (1 << chrsz) - 1;
    for (let i = 0; i < str.length * chrsz; i += chrsz) {
      bin[i >> 5] |= (str.charCodeAt(i / chrsz) & mask) << (24 - (i % 32));
    }
    return { bin, len: str.length * chrsz };
  }
  ...
  const { bin, len } = str2binb(encodedStr);
  return binb2hex(core_sha256(bin, len));
}
